...
raw_image_path = 'D:/abroad/summer/summer/data/luna16/'
candidates_file = 'D:/candidates.csv'
...
class CTScan(object):
    ...
    def read_mhd_image(self):
        ...
        print(self.path)
        print(self.filename)
        self.ds = sitk.ReadImage(path[0]) #读取mhd文件
        self.image = sitk.GetArrayFromImage(self.ds)
        

    def get_voxel_coords(self):#世界坐标转换为图像坐标
        """
        Converts cartesian to voxel coordinates
        """
        origin = self.ds.GetOrigin()#得到voxel坐标系原点在world中的坐标
        resolution = self.ds.GetSpacing()#得到空间结构，像素点之间的距离
    ...
    def normalizePlanes(self, npzarray):
        """
        Copied from SITK tutorial converting Houndsunits to grayscale units#将密度转换为灰度值
        """
        maxHU = 400.
        minHU = -1000.
...
def create_data(idx, outDir, X_data,  width = 50):
    ...
    outDir = a string representing destination#路径
    width (int) specify image size#说明数据的尺寸
    跟前面的模块不是一类了
    '''
    print(np.asarray(X_data.loc[idx])[0])
